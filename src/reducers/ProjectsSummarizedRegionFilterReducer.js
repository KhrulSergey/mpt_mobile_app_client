...
const getInitialState = (state, { districts, regions }) => {
    ...
    return { ...state, fetching: false, fetchingError: null, grouped, groupedDivided: divideArray(grouped), plain: regionListClone, selected, plainInitial: cloneDeep(regionListClone), groupedInitial: cloneDeep(grouped), updated_at };
};
...
export default (state = INITIAL_STATE, action) => {
    ...
    switch (type) {
        ...
        case FILTER_PROJECTS_SUMMARIZED_REGION_POPULATE:
            return populateFilter(state, payload);
        case FILTER_PROJECTS_SUMMARIZED_REGION_VISIBILITY_TOGGLE:
            return { ...state, ...payload, updated_at: Date.now() };
        case FILTER_PROJECTS_SUMMARIZED_REGION_TOGGLE:
            return toggleRegionSelection(state, payload);
        case FILTER_PROJECTS_SUMMARIZED_DISTRICT_TOGGLE:
            return toggleDistrictSelection(state, payload);
        case FILTER_PROJECTS_SUMMARIZED_REGION_RESET:
            return resetGroupedFilter(state);
        case FILTER_PROJECTS_SUMMARIZED_REGION_FILTER:
